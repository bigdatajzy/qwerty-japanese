/**
 * 解析 polyglot101 JLPT N4 形容词页 HTML（单表：形容詞 | ひらがな | ローマ字 | 中文 | 英文）
 * 用法：node scripts/parse-polyglot-n4-adjectives.mjs <path/to/n4-adjectives.html>
 */
...
if (!htmlPath) {
  console.error('用法: node scripts/parse-polyglot-n4-adjectives.mjs <下载的 n4-adjectives.html 路径>')
  process.exit(1)
}
...
chunks.forEach((chunk, idx) => {
  const name = `n4-adjectives-${String(idx + 1).padStart(2, '0')}.json`
...
const newSet = {
  id: 'jlpt-n4-adjectives',
  title: 'JLPT N4 形容词',
  level: 'N4',
  description: '来源：polyglot101.com N4 形容词页',
  wordCount: items.length,
  chunks: chunkPaths,
}

manifest.sets = [...manifest.sets.filter((s) => s.id !== 'jlpt-n4-adjectives'), newSet]
