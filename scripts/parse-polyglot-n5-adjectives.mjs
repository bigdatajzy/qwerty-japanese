/**
 * 解析 polyglot101 N5 形容词页 HTML（单表：形容词 | 平假名 | 罗马化 | 中文 | 英文）
 * 用法：node scripts/parse-polyglot-n5-adjectives.mjs <path/to/n5-adjectives.html>
 */
...
if (!htmlPath) {
  console.error('用法: node scripts/parse-polyglot-n5-adjectives.mjs <下载的 n5-adjectives.html 路径>')
  process.exit(1)
}
...
chunks.forEach((chunk, idx) => {
  const name = `n5-adjectives-${String(idx + 1).padStart(2, '0')}.json`
...
const newSet = {
  id: 'jlpt-n5-adjectives',
  title: 'JLPT N5 形容词',
  level: 'N5',
  description: '来源：polyglot101.com N5 形容词页',
  wordCount: items.length,
  chunks: chunkPaths,
}

manifest.sets = [...manifest.sets.filter((s) => s.id !== 'jlpt-n5-adjectives'), newSet]
