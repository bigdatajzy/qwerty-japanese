/**
 * 解析 polyglot101 N5 动词页 HTML（单表：Japanese Word | Kana | Pronunciation | 中文 | 英文）
 * 用法：node scripts/parse-polyglot-n5-verbs.mjs <path/to/n5-verbs.html>
 */
...
if (!htmlPath) {
  console.error('用法: node scripts/parse-polyglot-n5-verbs.mjs <下载的 n5-verbs.html 路径>')
  process.exit(1)
}
...
chunks.forEach((chunk, idx) => {
  const name = `n5-verbs-${String(idx + 1).padStart(2, '0')}.json`
...
const newSet = {
  ...
  title: 'JLPT N5 动词',
  level: 'N5',
  description: '来源：polyglot101.com N5 动词页',
  wordCount: items.length,
  chunks: chunkPaths,
}

manifest.sets = [...manifest.sets.filter((s) => s.id !== 'jlpt-n5-verbs'), newSet]
