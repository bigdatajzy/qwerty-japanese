/**
 * 解析 polyglot101 JLPT N4 动词页 HTML（单表：动词 | 平假名 | 罗马化 | 中文 | 英文）
 * 用法：node scripts/parse-polyglot-n4-verbs.mjs <path/to/n4-verbs.html>
 */
...
if (!htmlPath) {
  console.error('用法: node scripts/parse-polyglot-n4-verbs.mjs <下载的 n4-verbs.html 路径>')
  process.exit(1)
}
...
chunks.forEach((chunk, idx) => {
  const name = `n4-verbs-${String(idx + 1).padStart(2, '0')}.json`
...
const newSet = {
  ...
  title: 'JLPT N4 动词',
  level: 'N4',
  description: '来源：polyglot101.com N4 动词页',
  wordCount: items.length,
  chunks: chunkPaths,
}

manifest.sets = [...manifest.sets.filter((s) => s.id !== 'jlpt-n4-verbs'), newSet]
